...
def generate_cover_letter(cv, job_desc):
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "You are a helpful assistant. Create a cover letter based on the following CV and job description.",
            },
...
def show_homepage():
    ...
    st.write(
        "This web app is a demo for a job reccomender and cover-letter generator developed as part of the 2023 EF AI Hackathon in Cambridge."
    )
    ...
    uploaded_cv = st.file_uploader(
        "Upload your CV as a pdf ", type=["pdf"], accept_multiple_files=False
    )
    if uploaded_cv:
    ...
    if "parsed_cv" in locals():
        ...
        if st.button(f"Suggest me {num_jobs} jobs!"):
            ...
            st.session_state["top_job_titles_to_display"] = top_job_titles.head(
                num_jobs
            )
        ...
        if st.session_state["top_job_titles_to_display"] is not None:
            option = st.selectbox(
                ...
                st.session_state["top_job_titles_to_display"],
            )
            if st.button(f"Generate a cover letter for {option}"):
                ...
                job_description = top_jobs[top_jobs["Title"] == option][
                    "JobDescription"
                ].values[0]
